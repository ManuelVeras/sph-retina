...
def delta2bbox(rois,
    ...
    gxy = pxy + dxy_wh
    gwh = pwh * dwh.exp()
    ga  = pa + torch.rad2deg(da)
    bboxes = torch.cat([gxy, gwh, ga[:, None]], dim=-1)
    if clip_border: #and max_shape is not None:
        # TODO just for chenbin-format, i.e. ([0, 360), [0, 180), [0, 360], [0, 180]), deg
        bboxes[..., 0].clamp_(min=eps, max=360-eps)
        bboxes[..., 1].clamp_(min=eps, max=180-eps)
        bboxes[..., 2:4].clamp_(min=eps, max=180-eps)
        bboxes[..., 4].clamp_(min=-90+eps, max=90-eps)
