...
if __name__ == "__main__":
    ...
    kent_a4 = [3.5621, 0.1642, 8.8785, 3.0125, 5.3710]
    ...
    kent_b4 =  [1.1072e-01, 5.2726e-02, 7.2833e+00, 8.0882e+00, 2.2422e+00]
    kent_b5 = kent_a2
    ...
    max_value = 10  # Set your desired maximum value

    kent_a = (max_value - min_value) * torch.rand(10, 5) + min_value  # Generate random floats between min_value and max_value    kent_a[:, 4] = torch.rand(10) * (kent_a[:, 3] / 2)    
    kent_a[:, 1] = torch.minimum(kent_a[:, 1], kent_a[:, 0] / 2.1)  # Ensure 5th column is less than 4th column / 2
    kent_a[:, -3:] = kent_a[:, -3:].to(torch.int)
    print(kent_a)
    
    kent_b = (max_value - min_value) * torch.rand(10, 5) + min_value  # Generate random floats between min_value and max_value
    #kent_b[:, 4] = torch.rand(10) * (kent_b[:, 3] / 2)    
    kent_b[:, 1] = torch.minimum(kent_b[:, 1], kent_b[:, 0] / 2.1)  # Ensure 5th column is less than 4th column / 2
    kent_b[:, -3:] = kent_b[:, -3:].to(torch.int)
    print(kent_b)'''


    kent_a = torch.tensor([[6.3434, 3.0207, 7.0000, 9.0000, 7.0000]])
    kent_b = torch.tensor([[2.3307, 1.1098, 3.0000, 3.0000, 0.0000]])
    #kent_b = torch.tensor([[6., 3., 6.0000, 9.0000, 7.0000]])

    kent_loss_result = get_kld(kent_a, kent_b)
