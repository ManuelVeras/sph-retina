...
import sys
sys.path.append('/home/liuxinyuan/sph2obb')
import argparse
...
def bbox_map_eval(det_result, annotation, nproc=4, eval_map=None):
...
class ResultVisualizer:
    ...
    def __init__(self,
        ...
        assert box_version in [0, 4, 5]
        if box_version == 0:
            from mmdet.core.visualization import imshow_gt_det_bboxes
            from mmdet.core.evaluation import eval_map
        else:
            from sphdet.visualizers import imshow_gt_det_bboxes
            from sphdet.datasets.evaluation import eval_sph_bbox_map as eval_map
        self.imshow_gt_det_bboxes = imshow_gt_det_bboxes
    ...
    def detection_evaluate(self, dataset, results, topk=20):
        ...
        for i, (result, ) in enumerate(zip(results)):
            ...
            mAP = self.eval_map([result,], [data_info['ann_info'],])
            _mAPs[i] = mAP[0]
            prog_bar.update()
